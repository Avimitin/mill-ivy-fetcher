...
let
  scope = lib.makeScope newScope (self: {
    mill-ivy-fetcher = self.callPackage ./mill-ivy-fetcher/package.nix { };
    ...
    /**
      ...
      :::
    */
    ivy-gather = self.callPackage ./mill-ivy-fetcher/ivy-gather.nix { };
    ...
    */
    ...
    /**
      ...
      :::
    */
    publishMillJar = self.callPackage ./mill-ivy-fetcher/publish-mill-jar.nix { };

    configure-mill-env-hook = self.callPackage ./mill-ivy-fetcher/configure-mill-env.nix { };

    add-determinism-hook = self.callPackage ./mill-ivy-fetcher/add-determinism-hook.nix { };
  });
in
{
  inherit (scope)
